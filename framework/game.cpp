...
int getFieldx(vector3df position){
  int x = (int)position.Z/fieldsize;
  if (x<0) x=0;
  if (x>=dimx) x=dimx-1;
  return x;
} 
int getFieldy(vector3df position){
  int y = (int)position.X/fieldsize;
  if (y<0) y=0;
  if (y>=dimx) y=dimy-1;
  return y;
}
...
Sphere::Sphere(ISceneManager* smgr,
    ...
    sphere->setMaterialFlag(video::EMF_LIGHTING
			    , false);
	texture = driver->getTexture(texturepath+"fl-abyss.png");
...
// Returns the sphere's texture
ITexture* Sphere::getTexture(){
  return texture;
}

// Set a new texture for the sphere
void Sphere::setTexture(ITexture* newTexture) {
  texture = newTexture;
  sphere->setMaterialTexture(0,newTexture);
}

// Set a new texture for the sphere
void Sphere::setTexture(io::path newTexture){
	ITexture* alt_texture = driver->getTexture(texturepath+newTexture);
	texture = alt_texture;
	sphere->setMaterialTexture(0,alt_texture);
}
